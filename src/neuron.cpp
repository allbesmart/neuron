...
// AUC variables
int count = 0;                    //counter of detected objects
float eucl_dist;                  //3D euclidian distance
float P1_x[25], P1_y[25], P1_z[25];  //auxiliar vector for initial points position X, Y, Z
float P2_x, P2_y, P2_z;           //instantaneous centroid position X, Y, Z
float P3_x[25],P3_y[25],P3_z[25];    //auxiliar vector for end points position X, Y, Z
float travel_dist[25];             //travel distance
float lidar_dist[25];              //Lidar Distance
double ti, tf;                    //instant times
float velocity[25];                //velocity vector
float omega[25],alpha1[25],alpha2[25],theta[25],phi[25]; //angles
float dX[25],dY[25],d_angle[25];     //displacment angles
int a = 0;                        //auxiliar var
...
void Object3dDetector::classify() {
  ...
  for(std::vector<Feature>::iterator it = features_.begin(); it != features_.end(); ++it) {
    ...
    //---------------------------------------------------------------------------------
    // Euclidean distance from the centroids to lidar
    eucl_dist = sqrt(pow(pose.position.x,2)+pow(pose.position.y,2)+pow(pose.position.z,2));

    P2_x = pose.position.x; //centroid X
    P2_y = pose.position.y; //centroid Y
    P2_z = pose.position.z; //centroid Z
...
//*********************** MQTT *******
  mosquitto_lib_init();
  //sprintf(classCount, "%d", count);
  mosq = mosquitto_new("publisher_test",true,NULL);
  rc = mosquitto_connect(mosq,"localhost",1883,60);
  if (rc!=0){
    printf("Client not connected to Broker!= ERROR Code:%d\n",rc);
    mosquitto_destroy(mosq);
  }
  printf("Connected to Broker\n");
  double now = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count(); //timestamp

 //-------------------------------------------------
  int size_vec = sizeof(P3_x) / sizeof(float);
    for(int i = 0; i < count; i++){
      lidar_dist[i] = sqrt(pow(P3_x[i],2)+pow(P3_y[i],2)+pow(P3_z[i],2));
      omega[i] = asin(P3_z[i] / lidar_dist[i]);
      alpha1[i] = asin(P3_x[i] / (lidar_dist[i]*cos(omega[i])));
      alpha2[i] = acos(P3_y[i] / (lidar_dist[i]*cos(omega[i])));
      theta[i] = acos(P3_z[i] / (lidar_dist[i]));
      
      // displacement angles
      dX[i] = (P3_x[i] - P1_x[i]);
      dY[i] = (P3_y[i] - P1_y[i]);
      d_angle[i] = atan(dY[i]/dX[i]); 
      d_angle[i] = d_angle[i]*180/M_PI;

      if(d_angle[i] < 0)
        d_angle[i] = 180 - abs(d_angle[i]);
      ...
      travel_dist[i] = sqrt(pow(P3_x[i]-P1_x[i],2)+pow(P3_y[i]-P1_y[i],2)+pow(P3_z[i]-P1_z[i],2)); //travel distance
      velocity[i] = travel_dist[i] / (tf-ti);                                                    //velocity
      
      if(velocity[i] < 6.0){
        ROS_INFO("---");
        ROS_INFO("objectID:%d", i+1);
        ROS_INFO("lidarDistance:%.2f [m]", lidar_dist[i]);
        ROS_INFO("travelDistance:%.2f [m]", travel_dist[i]); 
        ROS_INFO("velocity: %.2f [m/s]", velocity[i]);
        ROS_INFO("omega:%.2f [degree]",omega[i]*180/M_PI);
        ROS_INFO("displacementAngle:%.2f",d_angle[i]);
        ROS_INFO("Positions:X:%.2f,Y:%.2f,Z:%.2f",P3_x[i],P3_y[i],P3_z[i]); 

        sprintf(classCount, "{\"objectID\":%d,\"lidarDistance\":%.2f,\"travelDistance\":%.2f,\"velocity\":%.2f,\"omega\":%.2f,\"displacementAngle\":%.2f,\"pX:\":%.2f,\"pY:\":%.2f,\"pZ:\":%.2f\"classLabel\": \"person\"}", i+1, lidar_dist[i], travel_dist[i],velocity[i],omega[i]*180/M_PI,d_angle[i],P3_x[i],P3_y[i],P3_z[i]); 
        strcat(buffer, classCount); 
      }
      P1_x[i] = P3_x[i];
      P1_y[i] = P3_y[i];
      P1_z[i] = P3_z[i];
    ...
    sprintf(classCount, "{\"classCount\":%d,\"timestamp\": %f,%s}", count, now,buffer);
    buffer[0] = '\0'; //clean the buffer
    
  mosquitto_publish(mosq,NULL,"jetson/lidar/count",strlen(classCount),classCount, 0, true);
